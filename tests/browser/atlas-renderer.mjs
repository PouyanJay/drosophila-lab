...
camera.position.set(0,0,20);camera.lookAt(0,0,-10);camera.updateMatrixWorld();const blocker=new THREE.Mesh(new THREE.PlaneGeometry(5,5),new THREE.MeshPhysicalMaterial({color:'white',opacity:1}));blocker.position.z=3;root.add(blocker);renderer.render(scene,camera);if(layer.pick(renderer,camera,80,80,160,160))throw Error('Picked hidden neuron through opaque surface');blocker.userData.bodyId='123';renderer.render(scene,camera);if(layer.pick(renderer,camera,80,80,160,160)?.bodyId!=='123')throw Error('Membrane surface lost neuron identity');delete blocker.userData.bodyId;blocker.visible=false;renderer.render(scene,camera);if(layer.pick(renderer,camera,80,80,160,160)?.bodyId!=='123')throw Error('Hidden surface still blocks selection');
layer.update({fibers:true,scope:'cns',group:-1,explode:0,hiddenClasses:['other']});renderer.render(scene,camera);if(layer.pick(renderer,camera,80,80,160,160))throw Error('Filtered cell remains pickable');layer.update({fibers:true,scope:'cns',group:-1,explode:0,hiddenClasses:[],colorMode:'mono'});renderer.render(scene,camera);if(layer.pick(renderer,camera,80,80,160,160)?.bodyId!=='123')throw Error('Show all failed to restore cell');
layer.update({fibers:true,scope:'cns',group:-1,explode:0,focusType:'Other type'});renderer.render(scene,camera);if(layer.pick(renderer,camera,80,80,160,160))throw Error('Type filter failed');layer.update({fibers:true,scope:'cns',group:-1,explode:0,focusType:'Fixture'});renderer.render(scene,camera);if(layer.pick(renderer,camera,80,80,160,160)?.bodyId!=='123')throw Error('Matching type missing');
