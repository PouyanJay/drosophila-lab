...
export default function DiscoveryPanel({
  ...
  const p = job?.progress,
    r = job?.result,
    records = r?.candidates || p?.candidates || [],
    best = r?.best || p?.best;
  ...
  const evidence = (
    <section className="discovery-evidence" aria-label="Discovery evidence">
      <div className="discovery-heading">
        ...
        {job && <span>{job.status}</span>}
      </div>
      {r && (
        <p>
          {r.outcome.replaceAll('_', ' ')} · {r.candidates.length} candidates evaluated.{' '}
          {r.improved
            ? 'The finalist passed the predefined confirmation rule.'
            : 'This run does not establish an improved brain.'}
        </p>
      )}
      {p?.trainingCurve?.length > 0 && (
        <>
          <h3>Most recent training curve</h3>
          <p>
            {p.candidate} · seed {p.seed}. Validation selects the saved weights.
          </p>
          <div style={{ height: 190, width: '100%', minWidth: 0 }}>
            <ResponsiveContainer>
              <LineChart data={p.trainingCurve}>
                <XAxis dataKey="step" stroke="#8092a9" />
                <YAxis stroke="#8092a9" />
                <Tooltip />
                <Line dataKey="trainLoss" stroke="#82b5fa" dot={false} />
                <Line dataKey="validationLoss" stroke="#c3acfa" dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </>
      ...
      {records.length > 0 && (
        <>
          ...
          <p>Validation scores guide search. These are not final test results.</p>
          <div className="discovery-candidates">
            {records.map((c: any) => (
              <button
                key={c.id}
                onClick={() => c.topology && onInspect(c.topology)}
                disabled={!c.topology}
              >
                <span>
                  <GitBranch size={14} />
                  {c.id}
                  <small>from {c.parent}</small>
                </span>
                <div className="discovery-bar">
                  <i
                    style={{ width: Math.max(1, (c.validationAccuracy ?? c.pilot) * 100) + '%' }}
                  />
                </div>
                <b>{((c.validationAccuracy ?? c.pilot) * 100).toFixed(1)}%</b>
                <small>{c.promoted ? 'Full evaluation' : 'Pilot only'}</small>
              </button>
            ))}
          </div>
        </>
      ...
      {best?.topology && (
        <div className="discovery-topology">
          <p>
            <strong>{best.topology.neurons.toLocaleString()} neurons</strong> ·{' '}
            {best.topology.edges.toLocaleString()} edges
          </p>
          <button
            onClick={() => {
              onInspect(best.topology);
            }}
          >
            Inspect modified source locations
          </button>
          <small>
            Atlas markers locate source neurons; they do not reconstruct new anatomical fibers.
          </small>
        </div>
      )}
      {r?.confirmation && (
        <>
          <h3>Fresh confirmation tests</h3>
          <div className="discovery-scores">
            <span>
              <b>{(r.confirmation.delta * 100).toFixed(1)} pp</b>
              {r.metricName || 'Accuracy'} gain
            </span>
            <span>
              <b>{r.confirmation.slowdown.toFixed(2)}×</b>Decision time
            </span>
            <span>
              <b>
                {r.confirmation.interval.map((v: number) => (v * 100).toFixed(1)).join(' to ')} pp
              </b>
          ...
          <div className="discovery-table">
            <table>
              ...
              <tbody>
                {r.confirmation.scenarios.map((s: any, i: number) => {
                  const rows = r.confirmation.runs,
                    mean = (name: string) =>
                      rows.reduce((n: number, x: any) => n + x[name].scenarios[i], 0) / rows.length;
                  return (
                    <tr key={i}>
                      <td>
                        {Object.entries(s)
                      ...
                      <td>{(mean('original') * 100).toFixed(1)}%</td>
                      <td>{(mean('candidate') * 100).toFixed(1)}%</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <p>{r.confirmation.caveat}</p>
        </>
      )}
      {r && (
        <>
          <p>Variant version: {r.variantId || 'No finalist'}</p>
          <a
  ...
  return (
    <section className="discovery-panel">
      ...
      <p>
        Runs in your local training service. Search for better task performance by changing the
        fruit-fly graph. Slower candidates are allowed within your limit.
      </p>
      <label>
        Task
        <select
          ...
          onChange={(e) =>
            onConfig({
              ...config,
              task: e.target.value as DiscoveryConfig['task'],
              taskParameters: '{}',
            })
          }
        >
      ...
      <div className="discovery-facts">
        <span>{config.candidates} candidates</span>
        <span>{config.maxCopies} added neurons maximum</span>
        <span>{Math.round(config.maxSeconds / 60)} minute budget</span>
        <span>Target +{config.minimumGain * 100} pp</span>
        <span>Up to {config.maxSlowdown}× slower</span>
      </div>
      <p className="discovery-protocol">
        Pilot → paired training → one finalist → fresh tests. No improvement is a valid outcome.
      </p>
      <details>
        <summary>Review all settings</summary>
        <p>
          Ask the chat to change these, or edit the validated specification here. Delay is in
          simulation steps.
        </p>
        <textarea
          ...
          <select value={selected || ''} onChange={(e) => onSelect(e.target.value || null)}>
            ...
            {jobs.map((j) => (
              <option key={j.id} value={j.id}>
                {names[j.config.task as keyof typeof names] || j.config.task} · {j.status} ·{' '}
      ...
      {job && (
        <div className="discovery-live">
          <strong>
            {job.status} · {p?.phase || 'waiting'}
          </strong>
          <p>
            {p?.candidate || ''}
            {p?.seed != null ? ' · seed ' + p.seed : ''}
          </p>
          {p && <progress value={p.elapsed} max={p.budgetSeconds} />}
