...
async function transaction(mode: IDBTransactionMode, fn: (s: IDBObjectStore) => void) {
  ...
  return new Promise<void>((resolve, reject) => {
    const t = d.transaction('pending', mode);
    fn(t.objectStore('pending'));
    t.oncomplete = () => resolve();
    t.onerror = () => reject(t.error);
    t.onabort = () => reject(t.error);
  });
}
export async function queueRecord(record: SavedRecord) {
  const revision = crypto.randomUUID();
  await transaction('readwrite', (s) => s.put({ ...record, revision, queuedAt: Date.now() }));
}
let flushing: Promise<void> | undefined;
export function flushRecords() {
  return (flushing ??= flush().finally(() => {
    flushing = undefined;
  }));
}
async function flush() {
  ...
  const rows: any[] = await new Promise((resolve, reject) => {
    const r = d.transaction('pending').objectStore('pending').getAll();
    r.onsuccess = () => resolve(r.result);
  ...
  for (const row of rows) {
    ...
    await transaction('readwrite', (s) => {
      const r = s.get(row.id);
      r.onsuccess = () => {
        if (r.result?.revision === row.revision) s.delete(row.id);
      };
    });
  }
}
export async function pendingRecords(): Promise<any[]> {
  const d = await db();
