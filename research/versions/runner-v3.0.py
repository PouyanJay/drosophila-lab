...
class FlyModel(nn.Module):
 def __init__(self,counts,classes,signs,labels,variant,device='cpu'):
  super().__init__();self.n=counts.shape[0];self.c=len(labels);self.device_name=device;self.variant=variant
  ...
  self.edge_count=w.nnz;self.removed=removed
...
def validate(config):
 ...
 if not isinstance(config.get('seeds'),list) or not 1<=len(config['seeds'])<=20 or any(not isinstance(s,int) or s<0 or s>1000000 for s in config['seeds']):raise ValueError('Invalid seeds')
...
def run(config,graphdir,out,device='cpu',threads=4):
 ...
 for vi,v in enumerate(variants):
  ...
  for seed in config['seeds']:
   ...
   changes={k:float(torch.linalg.vector_norm(p.detach().cpu()-init[k])) for k,p in model.named_parameters()};record={'seed':seed,'history':history,'bestStep':beststep,'validationLoss':best,'trainingSeconds':time.perf_counter()-tic,'neurons':model.n+model.graft,'biologicalNeurons':model.n,'engineeredUnits':model.graft,'edges':model.edge_count,'removedEdges':model.removed,'trainableParameters':sum(p.numel() for p in model.parameters() if p.requires_grad),'parameterChangeNorms':changes,'checkpoint':name,'checkpointSha256':hashlib.sha256((out/name).read_bytes()).hexdigest()};modelresult['seeds'].append(record);pending.append((vi,seed,name,record))
   del model,opt
