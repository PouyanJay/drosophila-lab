...
export function takeBrowserWorker() {
  const worker = prepared || new Worker('/research/full-worker.js', { type: 'module' });
  prepared = null;
  return worker;
}
export function prepareBrowserCompute(progress: (text: string) => void): Promise<string> {
  if (prepared) prepared.terminate();
  prepared = null;
  return new Promise((resolve, reject) => {
    ...
    worker.onmessage = ({ data }) => {
      if (data.type === 'progress')
        progress(
          data.data.phase +
            (data.data.download ? ' · ' + Math.round(data.data.download * 100) + '%' : ''),
        );
      ...
      if (data.type === 'ready') {
        ...
        worker.onerror = null;
        resolve(data.backend === 'webgpu' ? 'Ready · WebGPU' : 'Ready · CPU (WebGPU unavailable)');
      }
    };
