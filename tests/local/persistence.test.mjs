...
await pg.exec(
  readFileSync(path.join(root, 'supabase/migrations/20260915000000_local_workspace.sql'), 'utf8'),
);
await pg.exec('SET search_path=lab,public');
globalThis.__testPool = class {
  on() {}
  query(sql, values) {
...
const load = createModuleLoader({ pg: { Pool: globalThis.__testPool } });
const api = await load('@/app/api/records/route');
const outbox = await load('@/lib/client/local-outbox');
const sample = {
  plan: { task: 'beacon', duplicates: 32, prune: 0, seeds: 3, epochs: 12, budget: 'quick' },
  stage: 4,
  messages: [{ role: 'user', text: 'Remember a cue' }],
  execution: 'lab',
};
function request(method, body, query = '') {
  return new Request('http://localhost:3000/api/records' + query, {
    method,
...
test('independent conversations, full history, rename, and database reopen', async () => {
  assert.equal(
    (await post({ id: 'session-a', kind: 'agent-session', name: 'Cue memory', data: sample }))
      .status,
    200,
  );
  const long = {
  ...
  assert.equal(
    (await post({ id: 'session-b', kind: 'agent-session', name: 'Noise', data: long })).status,
    200,
  );
...
test('browser result preserves exact weights, trials, and provenance through restart', async () => {
  const result = JSON.parse(
    readFileSync(path.join(root, 'public/research/example-browser-run.json'), 'utf8'),
  );
  assert.equal(
    (await post({ id: 'result-a', kind: 'browser-run', name: 'Result', data: result })).status,
    200,
  );
  await pg.close();
  pg = await PGlite.create(directory);
  ...
  assert.equal(
    (await api.GET(request('GET', undefined, '?kind=agent-session&id=session-a'))).status,
    404,
  );
...
test('invalid evidence is rejected and all history pages remain accessible', async () => {
  assert.equal(
    (await post({ id: 'bad', kind: 'browser-run', name: 'Bad', data: { status: 'completed' } }))
      .status,
    400,
  );
  for (let i = 0; i < 55; i++)
    await post({ id: 'page-' + i, kind: 'agent-session', name: 'Paged ' + i, data: sample });
  const a = await get('?kind=agent-session&summary=1'),
    b = await get('?kind=agent-session&summary=1&offset=50');
...
test('outbox retains failed writes and retries successfully', async () => {
  ...
  try {
    ...
    assert.equal((await outbox.pendingRecords()).length, 0);
...
test('a save completing cannot discard a newer pending revision', async () => {
  ...
  try {
    ...
    globalThis.fetch = async () => {
      await outbox.queueRecord({ id: 'race', kind: 'agent-session', name: 'Second', data: sample });
...
test('discovery conversation retains its campaign, draft and review state', async () => {
  ...
  const data = {
    ...
    discoveryId: 'c4815ae4-f28d-4e0b-8372-de779a8e12f2',
  };
  assert.equal(
    (await post({ id: 'discovery-session', kind: 'agent-session', name: 'Memory discovery', data }))
      .status,
    200,
  );
  await pg.close();
  ...
  assert.equal(
    (
      await post({
        ...
        kind: 'agent-session',
        name: 'Invalid',
        data: { ...data, discoveryConfig: { ...defaultDiscovery, maxCopies: 1000000 } },
      })
    ).status,
    400,
  );
