...
async function api(path, body) {
  ...
  assert.ok(r.ok, d.error || d.detail);
  return d;
}
const health = await api('/api/lab/health');
...
for (let attempt = 0; attempt < 300 && job.status !== 'completed'; attempt++) {
  ...
  job = await api('/api/lab/jobs/' + job.id);
}
assert.equal(
  job.status,
  'completed',
  'Training must complete within five minutes on the test machine',
);
assert.deepEqual(job.config, config);
assert.ok(job.result);
assert.ok(job.artifacts.length);
const artifact = job.artifacts[0],
  r = await fetch(origin + '/api/lab/jobs/' + job.id + '/artifacts/' + artifact.name);
assert.equal(r.status, 200);
const sha = createHash('sha256')
  .update(Buffer.from(await r.arrayBuffer()))
  .digest('hex');
assert.equal(sha, artifact.sha256);
assert.equal(r.headers.get('X-Artifact-SHA256'), sha);
const again = await api('/api/lab/jobs/' + job.id);
assert.deepEqual(again.result, job.result);
console.log(
  'PASS: full-graph training, saved job, retained result and checksummed artifact download.',
);
