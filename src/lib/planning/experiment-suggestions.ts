export const experimentSuggestions = [
  {
    id: 'cue-memory',
    title: 'Remember a cue',
    description: 'Test recall after a distraction.',
    prompt: `Design a reproducible fruit-fly experiment: Remember a cue.
...
Next step: Use the selected training approach, explain its limits, and turn this brief into a complete reviewable plan. Ask only for important missing decisions. Check compute readiness before execution and wait for my instruction to start.`,
  ...
  {
    id: 'noisy-evidence',
    title: 'Decide under noise',
    description: 'Find a weak signal in noisy inputs.',
    prompt: `Design a reproducible fruit-fly experiment: Decide under noise.
...
export function suggestionPrompt(
  suggestion: (typeof experimentSuggestions)[number],
  execution: 'lab' | 'browser',
) {
  return execution === 'lab'
    ? suggestion.prompt
    : suggestion.prompt
        .replaceAll('32 / 16 / 32', '24 / 12 / 24')
        .replace(
          'Use the selected training approach',
          'Use browser readout training, with internal connections fixed',
        );
}
export function suggestionForPrompt(text: string) {
  return experimentSuggestions.find(
    (s) => s.prompt === text || suggestionPrompt(s, 'browser') === text,
  );
}
